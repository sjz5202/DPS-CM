...
class GaussianDiffusion:
    ...
    def p_sample_loop_xyind(self,
                      model,
                      x_start,
                      y_start,
                      measurement,
                      measurement_cond_fn_x,
                      measurement_cond_fn_y,
                      record,
                      save_root,
                      task,
                      model1=None):
        ...
        if(task=='inpainting'):
            mn=0
            img=self.q_sample(measurement, t=999-mn)
        elif(task=='super_resolution'):
            mn=0
            img=self.q_sample(measurement, t=999-mn)
            transform256 = T.Resize((256,256),interpolation=InterpolationMode.BICUBIC)
            img=transform256(img)
        for idx in pbar:
            ...
            y_out = self.p_sample(x=y, t=time, model=model1)
            x_noise=out['noise']
            y_noise=y_out['noise']
            noisy_measurement = self.q_sample(measurement, t=time)

            img, distance = measurement_cond_fn_x(x_t=out['sample'],
                                    measurement=measurement,
                                    noisy_measurement=noisy_measurement,
                                    x_prev=img,
                                    y_prev=y,
                                    x_0_hat=out['out']['pred_xstart'],
                                    y_0_hat=y_out['out']['pred_xstart'],
                                    x_eps=out['out']['eps'],
                                    y_eps=y_out['out']['eps'],
                                    out=out['out'],
                                    **self.__dict__)

            y, distance_y = measurement_cond_fn_y(y_t=y_out['sample'],
                                    measurement=measurement,
                                    noisy_measurement=noisy_measurement,
                                    x_prev=img_prev,
                                    y_prev=y,
            ...
            img = img.detach_()
            y=y.detach_()
            pbar.set_postfix({'distance_x': distance.item(),'distance_y': distance_y.item()}, refresh=False)
    ...
    def p_sample_loop_xyd(self,
        ...
        for idx in pbar:
            if(idx==999):
                ...
                y=y.requires_grad_()
                out = self.p_sample(x=img, t=time, model=model)
                y_out = self.p_sample(x=y, t=time, model=model)
            if(idx==0):
                break
            
            # Give condition.
            noisy_measurement = self.q_sample(measurement, t=time)
            time = torch.tensor([idx-1] * img.shape[0], device=device)
            ...
            y, distance_y = measurement_cond_fn_y(y_t=y_out['sample'],
                                      ...
                                      x_prev=img,
                                      y_prev=y,
                                      y_0_hat=y_out['pred_xstart'],
                                      x_eps=out['eps'],
                                      y_eps=y_out['eps'],
                                      guide_coef=y_out['guide_coef'],
                                      coef1=y_out['coef1'],
                                      coef2=y_out['coef2'])
            y=y.detach_()
            y=y.requires_grad_()
    ...
    def p_mean_variance(self, model, x, t):
        ...
        out = self.mean_processor.get_mean_and_xstart(x, t, model_output)
        model_variance, model_log_variance = self.var_processor.get_variance(model_var_values, t)

        assert out['mean'].shape == model_log_variance.shape == out['pred_xstart'].shape == x.shape

        return {'mean': out['mean'],
                'mean_change':out['mean_change'],
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t):
        ...
        out['alpha_bar_prev']=alpha_bar_prev
        out['alpha_bar']=alpha_bar

        noise = torch.randn_like(x)
        if t != 0:  # no noise when t == 0
            noise= torch.exp(0.5 * out['log_variance']) * noise
            sample += noise
        else:
            noise=0
        # return {'sample': sample, 'pred_xstart': out['pred_xstart'],'eps':out['eps'],'guide_coef':out['guide_coef'],'coef1':out['coef1'],\
        #         'coef2':out['coef2'],'coef3':out['coef3'],'coef4':out['coef4'],'coef5':out['coef5'],'noise':noise}
        return {'sample': sample,'noise':noise,'out':out}

...
class EpsilonXMeanProcessor(MeanProcessor):
    def __init__(self, betas, dynamic_threshold, clip_denoised):
        ...
        self.posterior_mean_coef1 = betas * np.sqrt(alphas_cumprod_prev) / (1.0-alphas_cumprod)
        self.posterior_mean_coef2 = (1.0 - alphas_cumprod_prev) * np.sqrt(alphas) / (1.0 - alphas_cumprod)
        self.guide_coef=np.sqrt(1.0 - alphas_cumprod)
        self.coef3_0=(1-alphas_cumprod)/alphas_cumprod
        self.coef4_0=1/np.sqrt(alphas)
    ...
    def q_posterior_mean(self, x_start, x_t, t):
        ...
        coef1 = extract_and_expand(self.posterior_mean_coef1, t, x_start)
        coef2 = extract_and_expand(self.posterior_mean_coef2, t, x_t)
        coef4 = extract_and_expand(self.coef4_0, t, x_t)
        self.coef1=coef1
        self.coef2=coef2
        self.coef3=self.coef3_0[t]
        return coef1 * x_start + coef2 * x_t,coef1 * x_start + coef2 * x_t - coef4*x_t
    ...
    def get_mean_and_xstart(self, x, t, model_output):
        pred_xstart = self.process_xstart(self.predict_xstart(x, t, model_output))
        mean,mean_change = self.q_posterior_mean(pred_xstart, x, t)

        return {'mean':mean,'mean_change':mean_change, 'pred_xstart':pred_xstart,'guide_coef':extract_and_expand(self.guide_coef,t,x),'coef1':self.coef1,'coef2':self.coef2,'coef3':self.coef3,'coef4':self.coef4,'coef5':self.coef5}
